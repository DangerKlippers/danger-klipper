...
def vibrate_axis(
    ...
    while freq <= max_freq + 0.000001:
        ...
        toolhead.cmd_M204(
            gcode.create_gcode_command("M204", "M204", {"S": accel})
        )
        L = 0.5 * accel * t_seg**2  # Distance for each half-cycle

        # Calculate move points based on axis direction (X, Y and Z)
        magnitude = math.sqrt(
            sum([component**2 for component in axis_direction])
        )
        normalized_direction = tuple(
            component / magnitude for component in axis_direction
        )
        dX, dY, dZ = (
            normalized_direction[0] * L,
            normalized_direction[1] * L,
            normalized_direction[2] * L,
        )
        nX = X + sign * dX
        nY = Y + sign * dY
        nZ = Z + sign * dZ
...
def vibrate_axis_at_static_freq(
    toolhead, gcode, axis_direction, freq, duration, accel_per_hz
):
    X, Y, Z, E = toolhead.get_position()
    sign = 1.0
