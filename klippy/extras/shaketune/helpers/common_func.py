...
def compute_mechanical_parameters(psd, freqs, min_freq=None):
    ...
    freq_above_half_power = freqs[idx_above - 1] + (
        half_power - psd[idx_above - 1]
...
def detect_peaks(
    ...
    indices,
    detection_threshold,
    relative_height_threshold=None,
    window_size=5,
    vicinity=3,
):
    ...
    smoothed_peaks = (
        np.where(
            (smoothed_data[:-2] < smoothed_data[1:-1])
            & (smoothed_data[1:-1] > smoothed_data[2:])
        )[0]
        + 1
    )
    smoothed_peaks = smoothed_peaks[
        smoothed_data[smoothed_peaks] > detection_threshold
    ]
    ...
    if relative_height_threshold is not None:
        ...
        for peak in smoothed_peaks:
            peak_height = smoothed_data[peak] - np.min(
                smoothed_data[
                    max(0, peak - vicinity) : min(
                        len(smoothed_data), peak + vicinity + 1
                    )
                ]
            )
...
def identify_low_energy_zones(power_total, detection_threshold=0.1):
    ...
    mean_energy = (
        np.mean(power_total) + (np.max(power_total) - np.min(power_total)) / 4
    )
