...
class MotorsConfigParser:
    def __init__(
        self,
        config,
        motors: List[str] = MOTORS,
        drivers: List[str] = TRINAMIC_DRIVERS,
    ):
        ...
        if motors is not None:
            for motor_name in motors:
                for driver in drivers:
                    tmc_object = self._printer.lookup_object(
                        f"{driver} {motor_name}", None
                    )
    ...
    def _create_motor(
        self, motor_name: str, driver: str, tmc_object: Any
    ) -> Motor:
    ...
    def _parse_klipper_config(self, motor: Motor, tmc_object: Any) -> None:
        ...
        motor.set_config(
            "microsteps",
            int(pconfig.status_raw_config[motor.name]["microsteps"]),
        )

        autotune_object = self._printer.lookup_object(
            f"autotune_tmc {motor.name}", None
        )
    ...
    def _parse_tmc_registers(self, motor: Motor, tmc_object: Any) -> None:
        ...
        for register in RELEVANT_TMC_REGISTERS:
            ...
            if (val is not None) and (
                register not in tmc_cmdhelper.read_registers
            ):
                # write-only register
                fields_string = self._extract_register_values(
                    tmc_cmdhelper, register, val
                )
    ...
    def _extract_register_values(self, tmc_cmdhelper, register, val):
        ...
        for _, field_name in reg_fields:
            field_value = tmc_cmdhelper.fields.get_field(
                field_name, val, register
            )
    ...
    def get_motor(self, motor_name: str) -> Optional[Motor]:
        return next(
            (motor for motor in self._motors if motor.name == motor_name), None
        )
