...
def compare_belts_responses(gcmd, config, st_process: ShakeTuneProcess) -> None:
    ...
    if motors_config_parser.kinematics == "corexy":
        ...
        filtered_config = [
            a for a in AXIS_CONFIG if a["axis"] in ("corexz_x", "corexz_z")
        ]
        ...
        raise gcmd.error(
            "Only CoreXY and CoreXZ kinematics are supported for the belt comparison tool!"
        )
    ConsoleOutput.print(
        f"{motors_config_parser.kinematics.upper()} kinematics mode"
    )
