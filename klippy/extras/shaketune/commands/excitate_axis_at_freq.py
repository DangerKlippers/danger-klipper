...
def excitate_axis_at_freq(gcmd, config, st_process: ShakeTuneProcess) -> None:
    create_graph = (
        gcmd.get_int("CREATE_GRAPH", default=0, minval=0, maxval=1) == 1
    )
    ...
    axis_config = next(
        (item for item in AXIS_CONFIG if item["axis"] == axis), None
    )
    if axis_config is None:
        raise gcmd.error(
            "AXIS selection invalid. Should be either x, y, a or b!"
        )
    ...
    if create_graph:
        ...
        if accel_chip is None:
            accel_chip = Accelerometer.find_axis_accelerometer(
                printer, "xy" if axis in {"a", "b"} else axis
            )
        k_accelerometer = printer.lookup_object(accel_chip, None)
        if k_accelerometer is None:
            raise gcmd.error(
                f"Accelerometer chip [{accel_chip}] was not found!"
            )
        accelerometer = Accelerometer(printer.get_reactor(), k_accelerometer)

    ConsoleOutput.print(
        f"Excitating {axis.upper()} axis at {freq}Hz for {duration} seconds"
    )
    ...
    vibrate_axis_at_static_freq(
        toolhead, gcode, axis_config["direction"], freq, duration, accel_per_hz
    )
    toolhead.dwell(0.5)
    ...
    if create_graph:
        accelerometer.stop_measurement(
            f"staticfreq_{axis.upper()}", append_time=True
        )
