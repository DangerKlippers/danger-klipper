...
def create_vibrations_profile(
    gcmd, config, st_process: ShakeTuneProcess
) -> None:
    ...
    if input_shaper is None:
        raise gcmd.error(
            "Input shaper is not configured! Please run the shaper calibration macro first."
        )

    motors_config_parser = MotorsConfigParser(
        config, motors=["stepper_x", "stepper_y"]
    )
    ...
    for curr_angle in main_angles:
        ...
        if current_accel_chip is None:
            current_accel_chip = Accelerometer.find_axis_accelerometer(
                printer, accel_axis
            )
        ...
        ConsoleOutput.print(
            f"Accelerometer chip used for this angle: [{current_accel_chip}]"
        )
        ...
        for curr_speed_sample in range(nb_speed_samples):
            ...
            toolhead.move(
                [mid_x - dX, mid_y - dY, z_height, E], feedrate_travel
            )
    ...
    creator.configure(
        motors_config_parser.kinematics, accel, motors_config_parser
    )
    st_process.run()
