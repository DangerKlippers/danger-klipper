...
def axes_shaper_calibration(gcmd, config, st_process: ShakeTuneProcess) -> None:
    ...
    min_freq = gcmd.get_float(
        "FREQ_START", default=res_tester.test.min_freq, minval=1
    )
    max_freq = gcmd.get_float(
        "FREQ_END", default=res_tester.test.max_freq, minval=1
    )
    ...
    if axis_input not in {"x", "y", "all"}:
        raise gcmd.error(
            "AXIS selection invalid. Should be either x, y, or all!"
        )
    scv = gcmd.get_float(
        "SCV", default=toolhead_info["square_corner_velocity"], minval=0
    )
    ...
    if len(test_points) > 1:
        raise gcmd.error(
            "Only one test point in the [resonance_tester] section is supported by Shake&Tune."
        )
    ...
    if (
        "minimum_cruise_ratio" in toolhead_info
    ):  # minimum_cruise_ratio found: Klipper >= v0.12.0-239
    ...
    filtered_config = [
        a
        for a in AXIS_CONFIG
        if a["axis"] == axis_input
        or (axis_input == "all" and a["axis"] in ("x", "y"))
    ]
    for config in filtered_config:
        # First we need to find the accelerometer chip suited for the axis
        accel_chip = Accelerometer.find_axis_accelerometer(
            printer, config["axis"]
        )
        if accel_chip is None:
            raise gcmd.error("No suitable accelerometer found for measurement!")
        accelerometer = Accelerometer(
            printer.get_reactor(), printer.lookup_object(accel_chip)
        )
        ...
        ConsoleOutput.print(
            f'{config["axis"].upper()} axis frequency profile generation...'
        )
    ...
    if (
        ...
        gcode.run_script_from_command(
            f"SET_VELOCITY_LIMIT ACCEL={old_accel} MINIMUM_CRUISE_RATIO={old_mcr}"
        )
    else:  # minimum_cruise_ratio not found: Klipper < v0.12.0-239
