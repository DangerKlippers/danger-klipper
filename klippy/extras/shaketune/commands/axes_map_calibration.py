...
def axes_map_calibration(gcmd, config, st_process: ShakeTuneProcess) -> None:
    ...
    if k_accelerometer is None:
        raise gcmd.error(
            "Multi-accelerometer configurations are not supported for this macro!"
        )
    pconfig = printer.lookup_object("configfile")
    current_axes_map = pconfig.status_raw_config[accel_chip].get(
        "axes_map", None
    )
    if (
        current_axes_map is not None
        and current_axes_map.strip().replace(" ", "") != "x,y,z"
    ):
        raise gcmd.error(
    ...
    if "minimum_cruise_ratio" in toolhead_info:
        ...
        old_mcr = None
        gcode.run_script_from_command(
            f"SET_VELOCITY_LIMIT ACCEL={accel} SQUARE_CORNER_VELOCITY=5.0"
        )
    ...
    toolhead.move(
        [mid_x - SEGMENT_LENGTH / 2, mid_y - SEGMENT_LENGTH / 2, z_height, E],
        feedrate_travel,
    )
    toolhead.dwell(0.5)
    ...
    toolhead.move(
        [
            mid_x + SEGMENT_LENGTH / 2,
            mid_y + SEGMENT_LENGTH / 2,
            z_height + SEGMENT_LENGTH,
            E,
        ],
        speed,
    )
    ...
    if (
        ...
        gcode.run_script_from_command(
            f"SET_VELOCITY_LIMIT ACCEL={old_accel} SQUARE_CORNER_VELOCITY={old_sqv}"
        )
