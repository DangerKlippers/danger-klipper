...
class ShakeTune:
    def __init__(self, config) -> None:
        ...
        result_folder = config.get(
            "result_folder", default="~/printer_data/config/ShakeTune_results"
        )
        result_folder_path = (
            Path(result_folder).expanduser() if result_folder else None
        )
        keep_n_results = config.getint(
            "number_of_results_to_keep", default=3, minval=0
        )
        ...
        self._config = ShakeTuneConfig(
            result_folder_path, keep_n_results, keep_csv, dpi
        )
        ...
        command_descriptions = {
            name: desc for name, _, desc in measurement_commands
        }
        for name, command, description in measurement_commands:
            gcode.register_command(
                f"_{name}" if show_macros else name, command, desc=description
            )
        ...
        if show_macros:
            ...
            try:
                ...
                raise config.error(
                    f"Cannot load Shake&Tune dummy macro {filename}"
                ) from err

            for gcode_macro in dummy_macros_cfg.get_prefix_sections(
                "gcode_macro "
            ):
                ...
                description = command_descriptions.get(
                    command, "Shake&Tune macro"
                )
                gcode_macro.fileconfig.set(
                    gcode_macro_name, "description", description
                )
    ...
    def cmd_EXCITATE_AXIS_AT_FREQ(self, gcmd) -> None:
        ConsoleOutput.print(
            f"Shake&Tune version: {ShakeTuneConfig.get_git_version()}"
        )
