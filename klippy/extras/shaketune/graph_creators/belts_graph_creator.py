...
PEAKS_DETECTION_THRESHOLD = (
    0.1  # Threshold to detect peaks in the PSD signal (10% of max)
)
...
class BeltsGraphCreator(GraphCreator):
    ...
    def clean_old_files(self, keep_results: int = 3) -> None:
        ...
        for old_file in files[keep_results:]:
            ...
            for suffix in {"A", "B"}:
                csv_file = (
                    self._folder / f"beltscomparison_{file_date}_{suffix}.csv"
                )
...
def pair_peaks(
    ...
    return PeakPairingResult(
        ...
        unpaired_peaks1=unpaired_peaks1,
        unpaired_peaks2=unpaired_peaks2,
    )
...
def compute_mhi(
    similarity_factor: float, signal1: SignalData, signal2: SignalData
) -> str:
    num_unpaired_peaks = len(signal1.unpaired_peaks) + len(
        signal2.unpaired_peaks
    )
    ...
    combined_unpaired_peaks = [
        (peak, signal1) for peak in signal1.unpaired_peaks
    ...
    if num_paired_peaks >= DC_MAX_PEAKS:
        mhi *= (
            DC_MAX_PEAKS / num_paired_peaks
        )  # Reduce MHI if more than ideal number of peaks
...
def plot_compare_frequency(
    ...
    for _, (peak1, peak2) in enumerate(signal1.paired_peaks):
        ...
        amplitude_offset = abs(
            ((signal2.psd[peak2[0]] - signal1.psd[peak1[0]]) / psd_highest_max)
            * 100
        )
        frequency_offset = abs(
            signal2.freqs[peak2[0]] - signal1.freqs[peak1[0]]
        )
        ...
        ax.plot(
            signal1.freqs[peak1[0]], signal1.psd[peak1[0]], "x", color="black"
        )
        ax.plot(
            signal2.freqs[peak2[0]], signal2.psd[peak2[0]], "x", color="black"
        )
        ax.plot(
...
def plot_versus_belts(
    ...
    green_boundary = ideal_line + (
        0.35 * max_psd * np.exp(-ideal_line / (0.6 * max_psd))
    )
    ax.fill_betweenx(
        ideal_line, ideal_line, green_boundary, color="green", alpha=0.15
    )
    ax.fill_between(
        ideal_line,
        ideal_line,
    ...
    ax.plot(
        signal1.psd, signal2.psd, color="dimgrey", marker="o", markersize=1.5
    )
    ax.fill_betweenx(
        signal2.psd, signal1.psd, color=KLIPPAIN_COLORS["red_pink"], alpha=0.1
    )
...
def compute_signal_data(
    data: np.ndarray, common_freqs: np.ndarray, max_freq: float
) -> SignalData:
    helper = shaper_calibrate.ShaperCalibrate(printer=None)
    calibration_data = helper.process_accelerometer_data(data)

    freqs = calibration_data.freq_bins[calibration_data.freq_bins <= max_freq]
    psd = calibration_data.get_psd("all")[
        calibration_data.freq_bins <= max_freq
    ]
...
def belts_calibration(
    ...
    try:
        ...
        ConsoleOutput.print(
            f"Warning: Unable to parse the date from the filename ({lognames[0]}, {lognames[1]})"
        )
        title_line2 = (
            lognames[0].split("/")[-1] + " / " + lognames[1].split("/")[-1]
        )
    fig.text(
        0.060,
        0.939,
    ...
    plot_versus_belts(
        ax3, common_freqs, signal1, signal2, signal1_belt, signal2_belt
    )
...
def main():
    ...
    opts.add_option(
        ...
        type="float",
        default=None,
        help="accel_per_hz used during the measurement",
    )
    opts.add_option(
        "-k",
        "--klipper_dir",
