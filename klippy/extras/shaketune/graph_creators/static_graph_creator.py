...
class StaticGraphCreator(GraphCreator):
    ...
    def configure(
        self, freq: float, duration: float, accel_per_hz: Optional[float] = None
    ) -> None:
        self._freq = freq
    ...
    def create_graph(self) -> None:
        if not self._freq or not self._duration or not self._accel_per_hz:
            raise ValueError(
                "freq, duration and accel_per_hz must be set to create the static frequency graph!"
            )
...
def plot_energy_accumulation(
    ax: plt.Axes, t: np.ndarray, bins: np.ndarray, pdata: np.ndarray
) -> None:
...
def main():
    ...
    opts.add_option(
        ...
        type="float",
        default=None,
        help="frequency maintained during the measurement",
    )
    opts.add_option(
        "-d",
        "--duration",
