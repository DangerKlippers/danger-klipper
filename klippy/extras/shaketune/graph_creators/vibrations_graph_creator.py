...
class VibrationsGraphCreator(GraphCreator):
    ...
    def configure(
        ...
        kinematics: str,
        accel: float,
        motor_config_parser: MotorsConfigParser,
    ) -> None:
    ...
    def create_graph(self) -> None:
        if not self._accel or not self._kinematics:
            raise ValueError(
                "accel and kinematics must be set to create the vibrations profile graph!"
            )

        lognames = self._move_and_prepare_files(
            glob_pattern="shaketune-vib_*.csv",
            min_files_required=None,
            custom_name_func=lambda f: re.search(
                r"shaketune-vib_(.*?)_\d{8}_\d{6}", f.name
            ).group(1),
        )
...
def compute_motor_profiles(
    ...
    for angle in measured_angles:
        ...
        sum_curve = np.sum(
            np.array([psds[angle][speed] for speed in psds[angle]]), axis=0
        )
        motor_profiles[angle] = np.convolve(
            sum_curve / len(psds[angle]), conv_filter, mode="same"
        )
        ...
        curve_area = (
            np.trapz(motor_profiles[angle], freqs)
    ...
    global_motor_profile = (
        weighted_sum_profiles / total_weight
        if total_weight != 0
        else weighted_sum_profiles
    )
...
def compute_dir_speed_spectrogram(
    ...
    spectrum_speeds = np.linspace(
        min(measured_speeds), max(measured_speeds), len(measured_speeds) * 6
    )
    spectrum_vibrations = np.zeros((len(spectrum_angles), len(spectrum_speeds)))

    def get_interpolated_vibrations(
        data: dict, speed: float, speeds: List[float]
    ) -> float:
        idx = np.clip(
            np.searchsorted(speeds, speed, side="left"), 1, len(speeds) - 1
        )
        ...
        return lower_vibrations + (speed - lower_speed) * (
            upper_vibrations - lower_vibrations
    ...
    for target_angle_idx, (cos_val, sin_val) in enumerate(
        zip(cos_vals, sin_vals)
    ):
        for target_speed_idx, target_speed in enumerate(spectrum_speeds):
            if kinematics == "cartesian" or kinematics == "corexz":
                ...
                speed_1 = np.abs(
                    target_speed * (cos_val + sin_val) * sqrt_2_inv
                )
                speed_2 = np.abs(
                    target_speed * (cos_val - sin_val) * sqrt_2_inv
                )

            vibrations_1 = get_interpolated_vibrations(
                data[measured_angles[0]], speed_1, measured_speeds
            )
            vibrations_2 = get_interpolated_vibrations(
                data[measured_angles[1]], speed_2, measured_speeds
            )
            spectrum_vibrations[target_angle_idx, target_speed_idx] = (
                vibrations_1 + vibrations_2
            )
...
def compute_angle_powers(spectrogram_data: np.ndarray) -> np.ndarray:
    ...
    extended_angles_powers = np.concatenate(
        [angles_powers[-9:], angles_powers, angles_powers[:9]]
    )
    convolved_extended = np.convolve(
        extended_angles_powers, np.ones(15) / 15, mode="same"
    )

    return convolved_extended[9:-9]


def compute_speed_powers(
    spectrogram_data: np.ndarray, smoothing_window: int = 15
) -> np.ndarray:
    ...
    data_arrays = np.stack(
        [min_values, max_values, var_values, vibration_metric]
    )
...
def filter_and_split_ranges(
    ...
    for start, end, energy in good_speeds:
        ...
        intersecting_peaks_indices = [
            idx
            for speed, idx in peak_speed_indices.items()
            if start_speed <= speed <= end_speed
        ]
        ...
        if not intersecting_peaks_indices:
            ...
            for peak_index in intersecting_peaks_indices:
                before_peak_end = max(
                    current_start, peak_index - deletion_range
                )
                if current_start < before_peak_end:
                    filtered_good_speeds.append(
                        (current_start, before_peak_end, energy)
                    )
    ...
    for current in sorted_ranges[1:]:
        last_merged_start, last_merged_end, last_merged_energy = merged_ranges[
            -1
        ]
...
def compute_symmetry_analysis(
    ...
    extended_spectrogram = np.concatenate(
        (spectrogram_data[-half_spectrogram_angles:], spectrogram_data), axis=0
    )

    # Calculate the split index directly within the slicing
    midpoint_angle = np.mean(measured_angles)
    split_index = int(
        midpoint_angle * (total_spectrogram_angles / 360)
        + half_spectrogram_angles
    )
    ...
    segment_1_flattened = extended_spectrogram[
        split_index - half_segment_length : split_index
    ].flatten()
    segment_2_flattened = extended_spectrogram[
        split_index : split_index + half_segment_length
    ].flatten()
...
def plot_angle_profile_polar(
    ...
    ax.plot(
        angles_radians, angles_powers, color=KLIPPAIN_COLORS["purple"], zorder=5
    )
    ax.fill(
        angles_radians,
        angles_powers,
    ...
    for _, (start, end, _) in enumerate(low_energy_zones):
        ...
        ax.fill_between(
            ...
            angles_powers[start:end],
            angles_powers.max() * 1.05,
            color="green",
            alpha=0.2,
        )
...
def plot_angular_speed_profiles(
    ...
    angle_settings = {
        ...
        45: (
            "A (45 deg)" if kinematics == "corexy" else "45 deg",
            "orange",
            10,
        ),
        135: (
            "B (135 deg)" if kinematics == "corexy" else "135 deg",
            "dark_orange",
            5,
        ),
    }
...
def plot_motor_profiles(
    ...
    for angle in main_angles:
        ...
        label = (
            f"{angle_settings[angle]} ({angle} deg)"
            if angle in angle_settings
            else f"{angle} deg"
        )
        ax.plot(
            freqs, motor_profiles[angle], linestyle="--", label=label, zorder=2
        )
    ...
    motor_fr, motor_zeta, motor_res_idx, lowfreq_max = (
        compute_mechanical_parameters(global_motor_profile, freqs, 30)
    )
    if lowfreq_max:
        ConsoleOutput.print(
    ...
    ax.plot(
        freqs[motor_res_idx],
        global_motor_profile[motor_res_idx],
        "x",
        color="black",
        markersize=10,
    )
    ...
    ax2.plot(
        [], [], " ", label=f"Motor resonant frequency (ω0): {motor_fr:.1f}Hz"
    )
    if motor_zeta is not None:
        ax2.plot(
            [], [], " ", label=f"Motor damping ratio (ζ): {motor_zeta:.3f}"
        )
    else:
...
def plot_motor_config_txt(
    fig: plt.Figure,
    motors: List[MotorsConfigParser],
...
def vibrations_profile(
    ...
    if kinematics == "cartesian" or kinematics == "corexz":
        ...
        raise ValueError(
            "Only Cartesian, CoreXY and CoreXZ kinematics are supported by this tool at the moment!"
        )
    ...
    measured_speeds = sorted(
        {
            speed
            for angle_speeds in psds_sum.values()
            for speed in angle_speeds.keys()
        }
    )
    ...
    for main_angle in main_angles:
        if main_angle not in measured_angles:
            raise ValueError(
                "Measurements not taken at the correct angles for the specified kinematics!"
            )
    ...
    sp_min_energy, sp_max_energy, sp_variance_energy, vibration_metric = (
        compute_speed_powers(spectrogram_data)
    )
    motor_profiles, global_motor_profile = compute_motor_profiles(
        target_freqs, psds, all_angles_energy, main_angles
    )

    # symmetry_factor = compute_symmetry_analysis(all_angles, all_angles_energy)
    symmetry_factor = compute_symmetry_analysis(
        all_angles, spectrogram_data, main_angles
    )
    ConsoleOutput.print(
        f"Machine estimated vibration symmetry: {symmetry_factor:.1f}%"
    )
    ...
    good_speeds = identify_low_energy_zones(
        vibration_metric, SPEEDS_VALLEY_DETECTION_THRESHOLD
    )
    if good_speeds is not None:
        deletion_range = int(
            SPEEDS_AROUND_PEAK_DELETION / (all_speeds[1] - all_speeds[0])
        )
        peak_speed_indices = {
            pspeed: np.where(all_speeds == pspeed)[0][0]
            for pspeed in set(peaks_speeds)
        }

        # Filter and split ranges based on peak indices, avoiding overlaps
        good_speeds = filter_and_split_ranges(
            all_speeds, good_speeds, peak_speed_indices, deletion_range
        )

        # Add some logging about the good speeds found
        ConsoleOutput.print(
            f"Lowest vibrations speeds ({len(good_speeds)} ranges sorted from best to worse):"
        )
        for idx, (start, end, _) in enumerate(good_speeds):
            ConsoleOutput.print(
                f"{idx+1}: {all_speeds[start]:.1f} to {all_speeds[end]:.1f} mm/s"
            )

    # Angle low energy valleys identification (good angles ranges) and print them to the console
    good_angles = identify_low_energy_zones(
        all_angles_energy, ANGLES_VALLEY_DETECTION_THRESHOLD
    )
    if good_angles is not None:
        ConsoleOutput.print(
            f"Lowest vibrations angles ({len(good_angles)} ranges sorted from best to worse):"
        )
    ...
    try:
        ...
        dt = datetime.strptime(
            f"{filename_parts[1]} {filename_parts[2].split('-')[0]}",
            "%Y%m%d %H%M%S",
        )
        ...
        ConsoleOutput.print(
            f"Warning: CSV filenames appear to be different than expected ({lognames[0]})"
        )
    ...
    if motors is not None and len(motors) == 2:
        ...
        if differences is not None and kinematics == "corexy":
            ConsoleOutput.print(
                f"Warning: motors have different TMC configurations!\n{differences}"
            )

    # Plot the graphs
    plot_angle_profile_polar(
        ax1, all_angles, all_angles_energy, good_angles, symmetry_factor
    )
    plot_vibration_spectrogram_polar(
        ax4, all_angles, all_speeds, spectrogram_data
    )
    ...
    plot_angular_speed_profiles(
        ax3, all_speeds, all_angles, spectrogram_data, kinematics
    )
    ...
    ax_logo.imshow(
        plt.imread(
            os.path.join(
                os.path.dirname(os.path.abspath(__file__)), "klippain.png"
            )
        )
    )
    ax_logo.axis("off")
...
def main():
    ...
    opts.add_option(
        ...
        dest="accel",
        default=None,
        help="accel value to be printed on the graph",
    )
    opts.add_option(
        "-f",
        "--max_freq",
    ...
    if options.output is None:
        opts.error(
            "You must specify an output file.png to use the script (option -o)"
        )
