...
class GraphCreator(abc.ABC):
    ...
    def _move_and_prepare_files(
        ...
        if not globbed_files:
            raise FileNotFoundError(
                f"no CSV files found in the /tmp folder to create the {self._type} graphs!"
            )
        if len(globbed_files) < min_files_required:
            raise FileNotFoundError(
                f"{min_files_required} CSV files are needed to create the {self._type} graphs!"
            )

        lognames = []
        for filename in sorted(
            globbed_files, key=lambda f: f.stat().st_mtime, reverse=True
        )[:min_files_required]:
            custom_name = (
                custom_name_func(filename)
                if custom_name_func
                else filename.name
            )
            new_file = (
                self._folder
                / f"{self._type.replace(' ', '')}_{self._graph_date}_{custom_name}.csv"
            )
    ...
    def _save_figure_and_cleanup(
        ...
        png_filename = (
            self._folder
            / f"{self._type.replace(' ', '')}_{self._graph_date}{axis_suffix}.png"
        )
