...
class ShaperGraphCreator(GraphCreator):
    ...
    def create_graph(self) -> None:
        if not self._scv:
            raise ValueError(
                "scv must be set to create the input shaper graph!"
            )
        ...
        self._save_figure_and_cleanup(
            fig, lognames, lognames[0].stem.split("_")[-1]
        )
...
def calibrate_shaper(
    ...
    fr, zeta, _, _ = compute_mechanical_parameters(
        calibration_data.psd_sum, calibration_data.freq_bins
    )
    ...
    try:
        ...
        compat = True
        shaper, all_shapers = helper.find_best_shaper(
            calibration_data, max_smoothing, ConsoleOutput.print
        )
...
def plot_freq_response(
    ...
    ax.axhline(
        y=peaks_threshold[0], color="black", linestyle="--", linewidth=0.5
    )
    ax.axhline(
        y=peaks_threshold[1], color="black", linestyle="--", linewidth=0.5
    )
    ax.fill_between(
        freqs,
        0,
...
def shaper_calibration(
    ...
    if len(datas) > 1:
        ConsoleOutput.print(
            "Warning: incorrect number of .csv files detected. Only the first one will be used!"
        )

    # Compute shapers, PSD outputs and spectrogram
    klipper_shaper_choice, shapers, calibration_data, fr, zeta, compat = (
    ...
    try:
        ...
        dt = datetime.strptime(
            f"{filename_parts[1]} {filename_parts[2]}", "%Y%m%d %H%M%S"
        )
        title_line2 = (
            dt.strftime("%x %X")
            + " -- "
            + filename_parts[3].upper().split(".")[0]
            + " axis"
        )
        if compat:
            ...
            title_line5 = (
                f"| Accel per Hz used: {accel_per_hz} mm/s²/Hz"
        ...
        ConsoleOutput.print(
            f"Warning: CSV filename look to be different than expected ({lognames[0]})"
        )
